...
module.exports = function (app,socket, io, db,listUsersOnline,passport) {
    ...
    app.get('/auth/facebook/callback',
        function (req, res, next) {
            passport.authenticate('facebook', {}, function (err, user) {
                ...
                req.session.save();
                if(!Cols.find(listUsersOnline,function(v){return v.email == user.email})){
                    var data = {
                        email: user.email,
                ...
                res.redirect("http://localhost:3000/#/chat-room");
            })(req, res, next);
        }
    );
    ...
    app.get('/api/account', function (req, res) {
        if(req.session.passport.user){
            if(!Cols.find(listUsersOnline,function(v){return v.email == req.session.passport.user.email})){
                var data = {
    ...
    socket.on('disconnect',function(){
        ...
        listUsersOnline.splice(Cols.findIndex(listUsersOnline,function(u){return u.email == socket.email }),1);
        updateListUsersOnline();
        var message = {
